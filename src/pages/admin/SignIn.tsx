...
const SignIn = () => {
  ...
  if ((status === Status.IDLE || status === Status.FULFILLED) && !isLoggedIn) {
    return (
        ...
        <div className="w-[384px] max-w-full bg-white pt-8 pb-12 px-4 md:px-8 rounded-lg">
          ...
          <form onSubmit={handleSubmit}>
            <fieldset className='mb-6'>
              <label htmlFor="email" className='font-medium block mb-1 text-sm md:text-base'>Email</label>
              <input type="text" id="email" readOnly value={email} onChange={e => setEmail(e.target.value)} className='border-[1px] border-slate-300 rounded-md block w-full py-2 px-4 text-sm md:text-base ' />
            ...
            <fieldset className='mb-6'>
              ...
              <input type="password" id="password" readOnly value={password} onChange={e => setPassword(e.target.value)} className='border-[1px] border-slate-300 rounded-md block w-full py-2 px-4 text-sm md:text-base' />
              {errors.password && <span className='text-xs md:text-sm text-red-500'>{errors.password}</span>}
            </fieldset>
            <button type="submit" className='bg-black text-white py-2 px-4 text-sm md:text-base rounded text-center cursor-pointer w-full'>Login</button>
          </form>
        </div>
      </section>
    )
